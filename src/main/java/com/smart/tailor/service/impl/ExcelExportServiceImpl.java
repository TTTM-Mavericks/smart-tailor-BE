...
public class ExcelExportServiceImpl implements ExcelExportService {
    ...
    public void exportCategoryMaterialData(List<MaterialResponse> materialResponses, HttpServletResponse response) throws IOException {
        ...
        style.setFont(font);
        style.setAlignment(HorizontalAlignment.CENTER);
        createCell(row, 0, "Category and Material List", style, sheet);
        sheet.addMergedRegion(new CellRangeAddress(0, 0, 0, 6));
        font.setFontHeightInPoints((short) 10);
        ...
        for(var materialResponse : materialResponses){
            Row rowSheet = sheet.createRow(rowIndex++);
            int countIndex = 0;
            createCell(rowSheet, countIndex++, materialResponse.getCategoryName(), styleData, sheet);
            createCell(rowSheet, countIndex++, materialResponse.getMaterialName(), styleData, sheet);
            createCell(rowSheet, countIndex++, materialResponse.getHsCode(), styleData, sheet);
